...
import cn.noblefull.module.impl.combat.Gapple;
...
public class Module implements Instance {
    ...
    public void setsuffix(String tag) {
        if (tag != null && !tag.isEmpty()) {
            String tagStyle = Optional.ofNullable(getModule(cn.noblefull.module.impl.display.ArrayList.class)).map(m -> m.tags.get()).orElse("").toLowerCase();
            if (getModule(cn.noblefull.module.impl.display.ArrayList.class).suffixColor.getValue()){
                switch (tagStyle) {
                    case "simple":
                        suffix = " " + tag;
                        break;
            ...
            } if (getModule(cn.noblefull.module.impl.display.ArrayList.class).style.is("Suffix")){
                switch (tagStyle) {
                    case "simple":
                        suffix = "§9 " + tag;
                        break;
    ...
    private void playClickSound(float volume) {
        if (mc.thePlayer != null) {
            switch (Client.INSTANCE.getModuleManager().getModule(InterFace.class).soundMode.getValue()) {
                ...
                case "Sigma":
                    if (State) {
                        SoundUtil.playSound(new ResourceLocation("noblefull/sounds/jello/activate.wav"), 1);
                    } else {
                        SoundUtil.playSound(new ResourceLocation("noblefull/sounds/jello/deactivate.wav"), 1);
                    }
                    break;
                case "Augustus":
                    if (State) {
                        SoundUtil.playSound(new ResourceLocation("noblefull/sounds/augustus/enable.wav"), 1);
                    } else {
                        SoundUtil.playSound(new ResourceLocation("noblefull/sounds/augustus/disable.wav"), 1);
                    }
                    break;
            }
        }
    ...
    public void setState(boolean state) {
        if (this.State != state) {
            ...
            if (state) {
                ...
                cn.noblefull.module.impl.display.Notification notificationModule = Client.Instance.getModuleManager().getModuleW(cn.noblefull.module.impl.display.Notification.class);
