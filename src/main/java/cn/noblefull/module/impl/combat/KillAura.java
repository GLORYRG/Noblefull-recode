...
import cn.noblefull.event.impl.events.player.*;
...
import cn.noblefull.module.impl.player.Blink;
...
import cn.noblefull.utils.rotation.RayCastUtil;
...
import static cn.noblefull.utils.pack.PacketUtil.sendPacket;
...
public class KillAura extends Module {
    ...
    public final ModeValue blockmode = new ModeValue("BlockMode",()->autoblock.get(), "Fake", new String[]{"Fake", "Grim","Blink","LEGIT", "Interact"});
        ...
        if (blinkab) {
            ...
            if (blocking) {
                if (this.serverSlot != newSlot) {
                    ...
                    //cn.noblefull.utils.pack.PacketUtil.sendPacket(new C09PacketHeldItemChange(this.serverSlot = newSlot));
                    this.swapped = true;
                }
                blocking = false;
            }
            cn.noblefull.utils.pack.PacketUtil.sendPacket(new C17PacketCustomPayload("孤月茫科技1234", new PacketBuffer(Unpooled.buffer())));
            blinkab = false;
        } else {
            ...
            if (result != null && result.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY && result.entityHit == target) {
                ...
                if (!blocking) {
                    blocking = true;
                    cn.noblefull.utils.pack.PacketUtil.sendPacket(new C08PacketPlayerBlockPlacement(mc.thePlayer.inventory.getCurrentItem()));
                }
            }
            blinkab = true;
                ...
                case "Blink":
                    if (this.swapped) {
                        ...
                        if (this.serverSlot != currentSlot) {
                            cn.noblefull.utils.pack.PacketUtil.sendPacket(new C09PacketHeldItemChange(this.serverSlot = currentSlot));
                        }
                        this.swapped = false;
                    }
