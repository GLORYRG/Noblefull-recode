...
import cn.noblefull.event.impl.events.player.StrafeEvent;
...
import cn.noblefull.module.impl.movement.Sprint;
...
import cn.noblefull.utils.player.PlaceData;
import cn.noblefull.utils.player.ScaffoldUtil;
import cn.noblefull.utils.player.SlotSpoofComponent;
import cn.noblefull.utils.render.BlockUtil;
import cn.noblefull.utils.render.PlaceInfo;
...
public class Scaffold extends Module {
    ...
    public ModeValue modeValue = new ModeValue("RotationMode","Normal",new String[]{"Normal","Telly","Telly2","Telly114514"});
        ...
        if (data != null && rotation.get() && mode.is("Normal") || mode.is("Telly") && canPlace && rotation.get()) {
            switch (modeValue.get()) {
                ...
                case "Telly114514":
                    ...
                    float warpAngTo180_float = MathHelper.wrapAngleTo180_float(f);
                    boolean z2 = Math.abs(warpAngTo180_float % 90.0f) <= 10f || Math.abs(warpAngTo180_float) >= 80f;

                    if (!mc.thePlayer.onGround) {
                        rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 90;
                    ...
                    if (!mc.gameSettings.keyBindRight.isKeyDown() || !mc.gameSettings.keyBindLeft.isKeyDown()) {
                        if (canPlace) {
                            if (mc.gameSettings.keyBindJump.isKeyDown()) {
                                rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 115;
                            } else {
                                rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 125;
                            }
                        } else if (mc.thePlayer.onGround) {
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw);
                        }
                    }
                    ...
                    if (mc.gameSettings.keyBindLeft.isKeyDown() && mc.gameSettings.keyBindForward.isKeyDown() && !z2) {
                        if (canPlace) {
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 155;
                        } else if (mc.thePlayer.onGround) {
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 150;
                        }
                    ...
                    if(mc.gameSettings.keyBindRight.isKeyDown() && mc.gameSettings.keyBindForward.isKeyDown() && z2){
                        if(canPlace){
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 133;
                        }else if(mc.thePlayer.onGround){
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 128;
                        }
                    ...
                    if (mc.gameSettings.keyBindLeft.isKeyDown() && mc.gameSettings.keyBindForward.isKeyDown()) {
                        if (canPlace) {
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw) - 127;
                        } else if (mc.thePlayer.onGround) {
                            rotations[0] = MovementUtil.getBindsDirection(mc.thePlayer.rotationYaw);
                        }
                    }
                    ...
                    for(int i =85; i > 60f; i--){
                        float[] tryRot = new float[]{rotations[0], i};
