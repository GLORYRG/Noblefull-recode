...
public class SplashScreen implements Instance {
    ...
    private static void drawScreen(float width, float height) {
        ...
        RenderUtil.drawLoadingCircle2(width / 2,height / 2 - 50,6,160,InterFace.mainColor.get().getRGB());
        RoundedUtil.drawRound(width / 2 - 75, height / 2 - 15, 150, 14, 6, new Color(221, 228, 255));
        RoundedUtil.drawRound(width / 2 - 75, height / 2 - 15, 150 * progress, 14, 6, InterFace.mainColor.get());
        FontManager.Bold.get(25).drawCenteredString("Welcome Noblefull Client",width / 2, height / 2 + 15, InterFace.mainColor.get().darker().getRGB());
        FontManager.Bold.get(20).drawCenteredString("Various calls are in progress please wait",width / 2, height / 2 + 35,InterFace.mainColor.get().darker().getRGB());
    }
}
