...
import cn.noblefull.utils.time.Timer;
...
public class Notification implements Instance {
    ...
    public Notification(String title, String message, Type type, long time) {
        image = new ResourceLocation("noblefull/notifications/" + type.name.toLowerCase() + ".png");
        this.title = title;
        this.time = (long) (1500);
    ...
    public void custom(double getY, double lastY) {
        ...
        animationX = width * animation.getOutput();
        posY = RenderUtil.animate(posY, getY);
        RenderUtil.drawRoundedRect((float) ((resolution.getScaledWidth() / 2) - width / 2), (float) posY - 2, (float) width, 10, 5,new Color(0, 0, 0, 100).getRGB());
        Semibold.get(18).drawCenteredString(message, resolution.getScaledWidth() / 2, posY, -1);
    }
    public void render(double getY, double lastY) {
        ...
        RenderUtil.startGlScissor(x1 - 4, y1 - 2, (int) width + 8,14);
            RoundedUtil.drawRound((float) x1, y1, (float) width , (float) 50, 2, ColorUtil.applyOpacity(new Color(Client.Instance.getModuleManager().getModule(InterFace.class).color()).darker(), (float) 0.3f));
        RenderUtil.stopGlScissor();

        FontManager.Icon.get(35).drawString(type.getIcon(), (float) (x1 + 9), (float) (y1 + 6f + (height - 6f) / 2f),  Client.Instance.getModuleManager().getModule(InterFace.class).color());

        Semibold.get(14).drawString(title, (x1 + 5), y1 + 4, Client.Instance.getModuleManager().getModule(InterFace.class).color());
        Semibold.get(16).drawString(message, (float) (x1 + imageWidth + 16),
                (float) ((float) y1 + 9 + (height - Semibold.get(16).getHeight()) / 2), Client.Instance.getModuleManager().getModule(InterFace.class).color());
