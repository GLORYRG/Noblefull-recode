package cn.noblefull.utils.rotation;
...
import cn.noblefull.utils.math.MathConst;
...
public class RotationUtil implements Instance {
    ...
    public static Rotation toRotation(Vec3 vec, boolean predict, Entity fromEntity) {
        Vec3 eyesPos = fromEntity.getPositionEyes(1.0F);

        if (predict) {
            eyesPos = eyesPos.addVector(fromEntity.motionX, fromEntity.motionY, fromEntity.motionZ);
        }
        ...
        float yaw = (float) MathHelper.wrapAngleTo180_float(
                (float) Math.toDegrees(Math.atan2(diffZ, diffX)) - 90.0F
        );
        float pitch = (float) MathHelper.wrapAngleTo180_float(
