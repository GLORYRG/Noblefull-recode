package cn.noblefull.utils.color;
...
import static cn.noblefull.utils.math.MathUtils.interpolateFloat;
import static cn.noblefull.utils.math.MathUtils.interpolateInt;
...
public class ColorUtil {
    ...
    public static Color colorSwitch(Color mainColor, Color secondColor, float time, int index, long timePerIndex, double speed) {
        return colorSwitch(mainColor, secondColor, time, index, timePerIndex, speed, 255.0D);
    }
    ...
    public static Color colorSwitch(Color mainColor, Color secondColor, float time, int index, long timePerIndex, double speed, double alpha) {
        long now = (long) (speed * (double) System.currentTimeMillis() + (double) ((long) index * timePerIndex));
        float redDiff = (float) (mainColor.getRed() - secondColor.getRed()) / time;
        float greenDiff = (float) (mainColor.getGreen() - secondColor.getGreen()) / time;
        float blueDiff = (float) (mainColor.getBlue() - secondColor.getBlue()) / time;
        ...
        float redInverseDiff = (float) (secondColor.getRed() - mainColor.getRed()) / time;
        float greenInverseDiff = (float) (secondColor.getGreen() - mainColor.getGreen()) / time;
        float blueInverseDiff = (float) (secondColor.getBlue() - mainColor.getBlue()) / time;
        int inverseRed = Math.round((float) mainColor.getRed() + redInverseDiff * (float) (now % (long) time));
        int inverseGreen = Math.round((float) mainColor.getGreen() + greenInverseDiff * (float) (now % (long) time));
        int inverseBlue = Math.round((float) mainColor.getBlue() + blueInverseDiff * (float) (now % (long) time));
