package cn.noblefull.utils.render;
...
import static cn.noblefull.utils.Instance.INSTANCE;
...
public class RenderUtil {
    ...
    public static int colorSwitch(Color mainColor, Color secondColor, float time, int index, long timePerIndex, double speed) {
        return colorSwitch(mainColor, secondColor, time, index, timePerIndex, speed, 255);
    }
    ...
    public static int colorSwitch(Color mainColor, Color secondColor, float time, int index, long timePerIndex, double speed, double alpha) {
        long now = (long) (speed * System.currentTimeMillis() + index * timePerIndex);

        float redDiff = (mainColor.getRed() - secondColor.getRed()) / time;
        float greenDiff = (mainColor.getGreen() - secondColor.getGreen()) / time;
        float blueDiff = (mainColor.getBlue() - secondColor.getBlue()) / time;
        ...
        float redInverseDiff = (secondColor.getRed() - mainColor.getRed()) / time;
        float greenInverseDiff = (secondColor.getGreen() - mainColor.getGreen()) / time;
        float blueInverseDiff = (secondColor.getBlue() - mainColor.getBlue()) / time;
        int inverseRed = Math.round(mainColor.getRed() + redInverseDiff * (now % (long) time));
        int inverseGreen = Math.round(mainColor.getGreen() + greenInverseDiff * (now % (long) time));
        int inverseBlue = Math.round(mainColor.getBlue() + blueInverseDiff * (now % (long) time));
