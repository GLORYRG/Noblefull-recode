package cn.noblefull.utils.render.shader;
...
import cn.noblefull.utils.render.OSUtil;
import cn.noblefull.utils.render.RoundedUtil;
import cn.noblefull.utils.render.shader.particle.ParticleEngine;
...
public class MainMenu implements Instance {
    ...
    static {
        try {
            shader = new GLSLSandboxShader("newbg.frag");
        } catch (Exception e) {
    ...
    public static void drawBackground(int guiWidth, int guiHeight, int mouseX, int mouseY) {
        if (OSUtil.supportShader()) {
            if (mc.currentScreen instanceof cn.noblefull.gui.MainMenu) {
            ...
            shader.useShader(sr.getScaledWidth() * sr.getScaleFactor(), sr.getScaledHeight() * sr.getScaleFactor(), mouseX, mouseY, (System.currentTimeMillis() - initTime) / 1000f, animation);
