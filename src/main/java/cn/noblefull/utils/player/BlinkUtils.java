...
import cn.noblefull.utils.math.Vector3d;
...
public class BlinkUtils {
    ...
    private static final List<Vector3d> positions;
    ...
    public static void doBlink() {
        ...
        if (!BlinkUtils.isStarted && BlinkUtils.mc.thePlayer != null) {
            synchronized (BlinkUtils.positions) {
                BlinkUtils.positions.add(new Vector3d(BlinkUtils.mc.thePlayer.posX, BlinkUtils.mc.thePlayer.getEntityBoundingBox().minY + BlinkUtils.mc.thePlayer.getEyeHeight() / 2.0f, BlinkUtils.mc.thePlayer.posZ));
                BlinkUtils.positions.add(new Vector3d(BlinkUtils.mc.thePlayer.posX, BlinkUtils.mc.thePlayer.getEntityBoundingBox().minY, BlinkUtils.mc.thePlayer.posZ));
            }
        ...
        if (BlinkUtils.mc.thePlayer != null) {
            synchronized (BlinkUtils.positions) {
                BlinkUtils.positions.add(new Vector3d(BlinkUtils.mc.thePlayer.posX, BlinkUtils.mc.thePlayer.posY, BlinkUtils.mc.thePlayer.posZ));
            }
        }
    }
        ...
        if (blinkSync) {
            try {
                ...
                while (!BlinkUtils.packets.isEmpty()) {
                    PacketUtil.sendPacket(BlinkUtils.packets.remove(0));
                }
            }
    ...
    static {
        ...
        positions = Collections.synchronizedList(new ArrayList<Vector3d>());
    }
}
